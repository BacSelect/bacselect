...
ARRAY="$REPO/validation/selector-v1/run_fresh_sequence_validation_retry.slurm"
...
[[ "$commit" == "$origin" ]] ||
    fail "HEAD does not match origin/main; push before network retrieval"
...
printf '%s\n' '===== corrected scheduler contract ====='
grep -nE \
    '#SBATCH --array|#SBATCH --nodelist|MAX_ATTEMPTS|failed-attempts' \
    "$ARRAY"

printf '%s\n' '===== submit corrected array ====='
...
dependency	afterok:$array_job
array_spec	1-31%2
nodelist	kscprod-bio8
fresh_targets	15326
fresh_batches	31
batch_size	500
final_batch_size	326
retry_contract	archive_failed_pre_zip_then_fresh_retry
supersedes_failed_execution_commit	b172cd104cc81113b1da862abadc6245c6dd9f76
EOF
...
printf 'PASS | submitted corrected fresh sequence validation | array=%s | aggregate=%s\n' \
